...
// docker
const gitCryptPromise = fetch(
        'https://github.com/dtschan/git-crypt-static/releases/download/v0.6.0/git-crypt'
).then(async (res) => {
        if (!res.ok) {
                const err = await res.text();
                throw new Error(`Failed to fetch "git-crypt": ${err}`);
        }
        const data = await res.buffer();
        return new FileBlob({ mode: 0o755, data });
});
...
export const build: BuildV3 = async ({
        ...
        const lambdaFiles: Files = {
                ...
                '.import-cache/bin/git-crypt': await gitCryptPromise,
